...
def download_video(video_url, start=None, end=None):
    ...
    try:
        ...
        if (start and start.strip() != "0:00") or (end and end.strip()):
            section_start = start if (start and start.strip() != "0:00") else ""
            section_end = end if end and end.strip() else ""
        if section_end:
        ...
        # Check for "error" anywhere in output
        if "error" in all_output:
            print("⚠️ Error detected in output — deleting file.")
            if os.path.exists(final_file):
                os.remove(final_file)
            return 'Download failed: Error text detected, file deleted.'

        # Check return code also
        if process.returncode != 0:
            print("⚠️ Non-zero exit code — deleting file.")
            if os.path.exists(final_file):
                os.remove(final_file)
            return f'Download failed: Non-zero exit code, file deleted.'

        # Sanity size check
        if not os.path.exists(final_file) or os.path.getsize(final_file) < 5 * 1024 * 1024:
            print("⚠️ File too small or missing — deleting file.")
            if os.path.exists(final_file):
                os.remove(final_file)
            return 'Download failed: File missing or too small, file deleted.'
...
def start():
    api = Api()
    window = webview.create_window('TrailHound v6.1', 'index.html', width=1600, height=1050)
