...
def fetch_youtube_links(query):
    ...
    for it in items:
        ...
        # Lazy-load resolution and duration later on request
    ...
    for name in os.listdir(path):
        ...
        if os.path.isdir(full_path):
            ...
            for ext in ['mp4', 'mkv', 'avi', 'mov', 'webm']:
                ...
                if os.path.isfile(trailer_path):  # check trailer
                    trailer_found = True
                    break
            if os.path.isfile(os.path.join(full_path, 'logo.png')) or os.path.isfile(os.path.join(full_path, 'clearlogo.png')):
                logo_found = True
                logo_count += 1
            folders.append({'name': base_name, 'found': trailer_found, 'logo': logo_found})

    # Sort: folders without trailer first
...
def start():
    api = Api()
    window = webview.create_window('YouTube Viewer/Downloader - TrailHound v3.8', 'index.html', width=2760, height=1035)
