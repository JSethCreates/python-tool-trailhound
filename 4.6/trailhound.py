# TrailHound v4.6 - Image path API added
import webview
import subprocess
import requests
import os
...
def get_images_for_folder(foldername):
    ...
    # Logo
    for file in ['logo.png', 'clearlogo.png']:
    ...
    for ext in exts:
        for file in [f"{base_name}-clearart.{ext}", f"clearart.{ext}"]:
            clearart_path = os.path.join(folder_path, file)
            if os.path.isfile(clearart_path):
                images['clearart'] = clearart_path
                break
    ...
    for ext in exts:
        for file in [f"{base_name}-fanart.{ext}", f"fanart.{ext}"]:
            fanart_path = os.path.join(folder_path, file)
            if os.path.isfile(fanart_path):
                images['fanart'] = fanart_path
                break
        if 'fanart' in images:
            break
...
def start():
    api = Api()
    window = webview.create_window('YouTube Viewer/Downloader - TrailHound v4.6', 'index.html', width=2304, height=1080)
